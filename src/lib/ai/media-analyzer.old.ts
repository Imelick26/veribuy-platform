...
/**
 * Estimates tire replacement cost for a specific vehicle using GPT-4o-mini.
...
Return JSON: { "perTireCents": <cost per tire in cents including install>, "totalCents": <total for ${tiresNeeded} tires in cents>, "reasoning": "<1 sentence explaining the estimate>" }`,
    ...
    return {
      ...
      reasoning: String(parsed.reasoning || `${tiresNeeded} tire replacement for ${vehicle.year} ${vehicle.make} ${vehicle.model}`),
    };
  } catch (err) {
