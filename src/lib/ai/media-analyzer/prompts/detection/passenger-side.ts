/**
 * Angle-specific prompt: Passenger Side
 * Phase 1 checklist for the full passenger-side profile photo.
 */

import type { VehicleInfo } from "../../types";

export function buildPassengerSideChecklist(
  vehicle: VehicleInfo,
  mileageStr: string,
): string {
  return `PASSENGER SIDE VIEW — Inspect the full passenger-side profile of this ${vehicle.year} ${vehicle.make} ${vehicle.model} (${mileageStr}). Be specific about location (e.g., "passenger rear door lower panel" not just "door"). Dirty or dusty surfaces are NOT defects — only flag actual damage, wear, or component issues. Dents show as light/shadow distortions on reflective surfaces; examine body line highlights carefully.

1. Passenger doors — dents, scratches, dings, or creases on front and rear door panels; note which door and where on the door (upper, center, lower)
...
4. Passenger mirror — glass cracked or delaminating, housing cracked or scraped, loose or wobbly mounting
